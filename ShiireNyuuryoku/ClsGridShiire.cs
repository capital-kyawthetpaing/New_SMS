...
//'明細部を構成するコントロールのうち TextBoxコントロール以外はカスタムコントロール(MeisaiControl)を使用してください。(Tabの検知のため)
    class ClsGridShiire :  GridBase.ClsGridBase
    {
        internal struct ST_DArray_Grid
        {
            ...
            internal string SyukkazumiFlg;
            internal string SyukkaSijizumiFlg;
            internal string PickingzumiFlg;
            internal string HikiatezumiFlg;
        ...
        internal void S_DispFromArray(int pStartRow, ref VScrollBar pScrool, int pStCtlRow = 0, int pEdCtlRow = -1)
        {
            ...
            for (w_CtlRow = pStCtlRow; w_CtlRow <= pEdCtlRow; w_CtlRow++)           // 画面上の明細番号
            {
                ...
                //マイナスPrice対応
                if (bbl.Z_Set(g_DArray[w_Row].PurchaseSu) < 0)
                {
                ...
                //マイナスPrice対応
                if (bbl.Z_Set(g_DArray[w_Row].PurchaseUnitPrice) < 0)
                {
                    g_MK_Ctrl[w_CtlCol, w_CtlRow].CellCtl.ForeColor = System.Drawing.Color.Red;
                }
                else
                ...
                //マイナスPrice対応
                if (bbl.Z_Set( g_DArray[w_Row].AdjustmentGaku) < 0)
                {
                    g_MK_Ctrl[w_CtlCol, w_CtlRow].CellCtl.ForeColor = System.Drawing.Color.Red;
                }
                else
                ...
                //マイナスPrice対応
                if (bbl.Z_Set(g_DArray[w_Row].PurchaseGaku) < 0)
                {
                    g_MK_Ctrl[w_CtlCol, w_CtlRow].CellCtl.ForeColor = System.Drawing.Color.Red;
                }
                else
