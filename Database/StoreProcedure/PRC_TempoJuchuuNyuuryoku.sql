...
--        BEGIN
            SELECT DH.JuchuuNO
                  ...
                  )AS PaymentPlanDate	--ñæç◊ïî.éxï•ó\íËì˙

                  ,CONVERT(varchar,DPC.CollectClearDate,111) AS D_CollectClearDate	--ñæç◊ïî.ì¸ã‡ì˙
                  ...
                  --ÅyData Area FooterÅz
                  ,DS.NouhinsyoComment
                  ,MAX(CONVERT(varchar,A.SalesDate,111)) OVER() AS D_Sales_SalesDate	--îÑè„ì˙
                  ,SUM(B.CollectGaku) OVER() AS CollectGaku	--ì¸ã‡äz
                  ,MAX(CONVERT(varchar,C.BillingCloseDate,111)) OVER() AS BillingCloseDate	--êøãÅí˜ì˙
                  ,MAX(CONVERT(varchar,C.CollectPlanDate,111)) OVER() AS CollectPlanDate	--ÅiêøãÅéûÅjì¸ã‡ó\íËì˙
                  ,MAX(CONVERT(varchar,D.CollectClearDate,111)) OVER() AS CollectClearDate --ç≈èIì¸ã‡ì˙
...
--                SELECT M.JuchuuNO,M.JuchuuRows,SUM(H.CollectGaku) AS CollectGaku	2019.10.23 chg
              ...
              --ñæç◊ïîÇÃì¸ã‡ì˙ÇãÅÇﬂÇÈÇΩÇﬂÇÃÉTÉuÉNÉGÉä
...
BEGIN
    ...
    --ä˘Ç…îÑè„çœÇ›åxçê
    --à»â∫ÇÃèåèÇ≈ÉåÉRÅ[ÉhÇ™Ç†ÇÍÇŒîÑè„çœÇ∆ÇµÇƒåxçêÉÅÉbÉZÅ[ÉWÇï\é¶Ç∑ÇÈ
    ...
    --ä˘Ç…èoâ◊çœÇ›åxçê
    --à»â∫ÇÃèåèÇ≈ÉåÉRÅ[ÉhÇ™Ç†ÇÍÇŒèoâ◊çœÇ∆ÇµÇƒåxçêÉÅÉbÉZÅ[ÉWÇï\é¶Ç∑ÇÈ
    ...
    --ä˘Ç…èoâ◊éwé¶çœÇ›åxçê
    --à»â∫ÇÃèåèÇ≈ÉåÉRÅ[ÉhÇ™Ç†ÇÍÇŒèoâ◊éwé¶çœÇ∆ÇµÇƒåxçêÉÅÉbÉZÅ[ÉWÇï\é¶Ç∑ÇÈ
    SELECT @CNT = COUNT(A.Number)
    ...
    --ä˘Ç…ÉsÉbÉLÉìÉOÉäÉXÉgäÆóπçœÇ›åxçê
    --à»â∫ÇÃèåèÇ≈ÉåÉRÅ[ÉhÇ™Ç†ÇÍÇŒÉsÉbÉLÉìÉOçœÇ∆ÇµÇƒåxçêÉÅÉbÉZÅ[ÉWÇï\é¶Ç∑ÇÈ
    SELECT @CNT = COUNT(A.Number)
    ...
    --ä˘Ç…édì¸çœÇ›åxçê
    --à»â∫ÇÃèåèÇ≈ÉåÉRÅ[ÉhÇ™Ç†ÇÍÇŒî≠íççœÇ∆ÇµÇƒåxçêÉÅÉbÉZÅ[ÉWÇï\é¶Ç∑ÇÈ
    SELECT @CNT = COUNT(A.JuchuuNO)
...
--  [SetRows] [tinyint] ,
    ...
    [ZaikoKBN] [tinyint] ,
    [TemporaryNO] [varchar](11) ,	--âºà¯ìñî‘çÜ
    [ExpressFLG] [tinyint] ,
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            --ç≈èâÇ…à¯ìñèÓïÒÇÉNÉäÉAÇµÅAç›å…Ç…ñﬂÇ∑ 
            --ÅyD_StockÅz 
            UPDATE [D_Stock]
            ...
            ;
             ...
             --20200225Å@ÉeÅ[ÉuÉãì]ëóédólÇiáA
            ...
            ;
             ...
             --ÉeÅ[ÉuÉãì]ëóédólÇgÅ'ÅiçÌèúèàóùÅj
             --ÅyD_ReserveÅz
        ...
        -- ========= ÉãÅ[Évì‡ÇÃé¿ç€ÇÃèàóù Ç±Ç±Ç©ÇÁ===
            --IF @ZaikoKBN = 1
            IF @DirectFlg = 0	--íÜêgÇÕDirectFlg
            BEGIN
                EXEC Fnc_Reserve_SP
                    @AdminNO,
                    @JuchuuDate,
                    ...
                    @Suryo,
                    1,  --@DenType
                    @tblJuchuuNo,  --êVãKéûÇÃéÛíçî‘çÜÇçÃî‘Ç∑ÇÈÇ◊Ç´ÅH
                    @JuchuuRows,
                    @KariHikiateNo,
                    @Result OUTPUT,
                    @Error OUTPUT,
    ...
    BEGIN
        ...
        BEGIN
        -- ========= ÉãÅ[Évì‡ÇÃé¿ç€ÇÃèàóù Ç±Ç±Ç©ÇÁ===
        	--ÉeÉjÉbÉNÇÃèÍçáÇÕçsí«â¡éûÇ‡ÅBÉeÉjÉbÉNÇ≈Ç»Ç¢èÍçáÇÕêVãKéûÇÃÇ›1âÒçÃî‘
            ...
            BEGIN
             ...
             VALUES
                ...
                --ÉeÉjÉbÉNÇÃèÍçáÅiìXï‹éÛíçì¸óÕÇÃåãâ Çèoâ◊éwé¶Ç…ï\é¶Ç∑ÇÈÅj
                --M_Control.TennicFLGÅÅ1
                IF @Tennic = 1
                BEGIN
                	
                    --ÅyD_DeliveryPlanÅzîzëóó\íËèÓïÒÅ@Tableì]ëóédólÇj
                    --ì`ï[î‘çÜçÃî‘
                    EXEC Fnc_GetNumber
                        19,             --inì`ï[éÌï  19
                        @JuchuuDate,      --inäÓèÄì˙
                    ...
                    SELECT
                            @DeliveryPlanNO
                           ,1 AS DeliveryKBN    --(1:îÃîÑÅA2:ëqå…à⁄ìÆ)
                           ,@JuchuuNO   AS Number
                           ,@DeliveryName
                ...
                END
            END		--ÉwÉbÉ_ÇINSERTÇ∑ÇÈÇ©Ç«Ç§Ç©ÇÃèåè
            ...
            BEGIN
                ...
                INSERT INTO [D_JuchuuDetails]
                     ...
                     SELECT @JuchuuNO                         
                      ...
                      AND (@Tennic = 0  OR (@Tennic = 1 AND tbl.JuchuuRows = @JuchuuRows))  --Åö
                      ;
            END

            --èCê≥éûÇÃÇ›ÅiçsçÌèúÉfÅ[É^Ç∆èCê≥ÉfÅ[É^ÇÃçXêVÅj
            ...
            BEGIN 
                --ÅyD_ReserveÅzINSERTèàóùÅ@éÛíççsêîï™ÇÃÉåÉRÅ[ÉhÇçÏê¨(à¯ìñç›å…ÉåÉRÅ[ÉhÇÃåèêîï™)
                --ÉJÅ[É\ÉãíËã`
                ...
                -- ========= ÉãÅ[Évì‡ÇÃé¿ç€ÇÃèàóù Ç±Ç±Ç©ÇÁ===
                    ...
                    INNER JOIN  @Table tbl ON tbl.JuchuuRows = @GNO         --ÅöTennicÇÃèÍçáÇ≈Ç‡âÊñ ÇÃçsî‘çÜÇ™äiî[Ç≥ÇÍÇƒÇ¢ÇÈ
                   ...
                   --     INNER JOIN  @Table tbl ON A.TemporaryNO = (SELECT HNO FROM @TAB WHERE GNO = tbl.JuchuuRows)
                    ...
                    UPDATE D_JuchuuDetails SET
                        ...
                        ,HikiateFlg = (CASE WHEN D_JuchuuDetails.HikiateSu + A.ReserveSu >= D_JuchuuDetails.JuchuuSuu THEN 1
                                    ...
                                    ELSE 0 END )
                    FROM D_TemporaryReserve A
                    INNER JOIN @Table tbl ON tbl.JuchuuRows = @GNO      --Åö
                    WHERE A.TemporaryNO = @HNO
                    AND A.KeySEQ = @KeySEQ                    
            ...
            IF @Tennic = 1
            BEGIN
                --ÅyD_DeliveryPlanDetailsÅzîzëóó\íËñæç◊Å@Tableì]ëóédólÇk
                ...
                --èoâ◊ó\íËÅ@D_DeliveryPlan Tableì]ëóédólÇj
                UPDATE [D_DeliveryPlan] SET
                    HikiateFLG = 1
    ...
    IF @OperateMode = 3    --çÌèú
    BEGIN
        SET @OperateModeNm = 'çÌèú';
        
        --ÅyD_MitsumoriÅz
        UPDATE D_Mitsumori
        SET JuchuuFLG = 0	--éÛíçê¨ñÒFLGÅÅÇOÇ…UPDATE
           ,[UpdateOperator] =  @Operator  
           ,[UpdateDateTime] =  @SYSDATETIME
        WHERE MitsumoriNO = @MitsumoriNO
        ;
