...
CREATE PROCEDURE [dbo].[D_Mail_SelectAll](
...
BEGIN
    ...
    WHERE CONVERT(varchar,DM.MailDateTime,111) >= @MailDateFrom
    AND CONVERT(varchar,DM.MailDateTime,111) <= @MailDateTo
    AND DM.MailDateTime >= (CASE WHEN ISNULL(@MailTimeFrom,'') <> '' THEN CONVERT(datetime,@MailTimeFrom) ELSE ISNULL(DM.MailDateTime,'') END)
    AND DM.MailDateTime <= (CASE WHEN ISNULL(@MailTimeTo,'') <> '' THEN CONVERT(datetime,@MailTimeTo) ELSE ISNULL(DM.MailDateTime,'') END)
