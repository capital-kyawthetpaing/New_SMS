...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			DELETE  from M_ItemOrderPrice  where MakerItem in (select MakerItem from M_ITEM ms where ms.ITemCD in (select ITemCD from #tempItem))
		 ...
		 --    TP.MainVendorCD
			...
			--from #tempItem tp
			--inner join M_SKU ms on tp.ITemCD = ms.ITemCD and tp.ChangeDate = ms.ChangeDate and ms.DeleteFlg = 0
			--where (tp.MainVendorCD <> '' and tp.MainVendorCD is not null)

				select MainVendorCD ,Store ,  MakerItem, changedate, max(Rate) ,Max(OP), max(l), max(k ), max(iopt),MAX(idt),max(uopt),max(udt)   from (
			select  distinct
		     TP.MainVendorCD 
			,'0000' as Store
			...
			where (tp.MainVendorCD <> '' and tp.MainVendorCD is not null) 
			) adc 		where changedate is not null	group by MainVendorCD , Store , MakerItem, changedate order by MainVendorCD, MakerItem ,changedate -- having Nom >1
