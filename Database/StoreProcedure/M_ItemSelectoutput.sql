...
BEGIN
	...
	IF (@OrderAttentionCD = -1)
		...
		,(select top 1 BrandName  from   M_Brand mb where mb.BrandCD =fim.BrandCD)  as 'ブランド名' 
		,fim.makerItem as 'メーカー商品CD'
		,(fim.SizeCount ) as '展開サイズ数'
		,fim.ColorCount as '展開カラー数'
		,fim.TaniCD as '単位CD'
	, (select top 1 LEFT (Char1, 20) from M_MultiPorpose mp where mp.[ID] ='201' and [Key]=fim.TaniCD) as '単位名'
		,fim.SportsCD as  '競技CD'
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='202' and [Key]= fim.SportsCD) as '競技名'
		,fim.SegmentCD as '商品分類CD'
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='203'  and [Key]= fim.SegmentCD ) as '分類名'
		,fim._ExhibitionSegmentCD as 'セグメントCD'
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='206'  and [Key]= fim._ExhibitionSegmentCD) as 'セグメント名'
		...
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='311' and [Key]=fim.ReserveCD) as '予約品区分名'
		,fim.NoticesCD as '特記区分'
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='310'  and [Key]=fim.NoticesCD) as '特記区分名'
		,fim.PostageCD as '送料区分'
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='309'  and [Key]=fim.PostageCD) as '送料区分名'
		,fim.ManufactCD as '要加工品区分'
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='312'  and [Key]=fim.ManufactCD) as '要加工品区分名'
		,fim.ConfirmCD as '要確認品区分'
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='313'  and [Key]=fim.ConfirmCD) as '要確認品区分名'
	...
	, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] ='319' and [Key]=fim.OrderAttentionCD) as '発注注意区分名'
