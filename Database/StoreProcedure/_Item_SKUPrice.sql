...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			update  msp
			set 
			 --msp.TanKaCD= '0000000000000' 
			--,msp.StoreCD = '0000' 
			--,msp.AdminNo = msk.AdminNO
			--,msp.SKUCD = msk.SKUCD
			--,msp.ChangeDate =tim.ChangeDate
			...
			,msp.DeleteFlg =0
			,msp.UsedFlg =0
			--,msp.InsertOperator = @Opt
			--,msp.InsertDateTime =@Date
			,msp.UpdateOperator = @Opt
			,msp.UpdateDateTime =@Date
			--select distinct  msk.AdminNO , tim.ChangeDate
			from   M_SKUPrice msp
			inner join F_SKU(getdate()) msk on msk.AdminNO = msp.AdminNO 
			inner join  #tempItem tim on tim.ITemCD = msk.ITemCD and tim.ChangeDate = msk.ChangeDate and msk.DeleteFlg =0


			insert into M_SKUPrice (
			TankaCD , StoreCD , AdminNO , SKUCD , ChangeDate , TekiyouShuuryouDate , PriceWithTax , PriceWithoutTax
			,GeneralRate , GeneralPriceWithTax ,GeneralPriceOutTax , MemberRate , MemberPriceWithTax , MemberPriceOutTax
			,ClientRate , ClientPriceWithTax, ClientPriceOutTax , SaleRate , SalePriceWithTax , SalePriceOutTax
			,WebRate, WebPriceWithTax, WebPriceOutTax ,Remarks , DeleteFlg, UsedFlg
			...
			from F_SKU(getdate()) fsk 
			inner join #tempItem tim on fsk.ITemCD = tim.ITemCD and tim.ChangeDate =fsk.ChangeDate and fsk.DeleteFlg = 0
			where fsk.ITemCD not in (
				select msk.ITemCD from   M_SKUPrice msp
			inner join F_SKU(getdate()) msk on msk.AdminNO = msp.AdminNO 
			inner join  #tempItem tim on tim.ITemCD = msk.ITemCD and tim.ChangeDate = msk.ChangeDate and msk.DeleteFlg =0
			)
