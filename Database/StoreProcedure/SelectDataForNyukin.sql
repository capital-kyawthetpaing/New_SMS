...
CREATE PROCEDURE [dbo].[SelectDataForNyukin](
...
BEGIN
    ...
    IF @RdoSyubetsu = 2
    BEGIN
    	--NyuukinNyuuryoku_入金入力　②明細単位　b入金顧客
    	--画面転送表05 新規で表示ボタン押下時
        SELECT CONVERT(varchar,DB.BillingCloseDate,111) AS BillingCloseDate
              ...
              ,ISNULL(DCBD.CollectAmount,0) AS D_CollectAmount		--入金済額
              ,ISNULL(DBM.BillingGaku,0) -ISNULL(DCBD.CollectAmount,0) AS ConfirmAmount --今回入金額
