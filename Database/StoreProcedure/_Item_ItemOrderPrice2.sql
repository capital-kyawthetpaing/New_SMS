...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		from 
		M_ItemOrderPrice mio
		inner join F_SKU(getdate()) fs on fs.MakerItem = mio.MakerItem and fs.DeleteFlg =0
		inner join #tempItem tim on fs.ITemCD = tim.ITemCD and mio.ChangeDate = tim.ChangeDate
		where mio.VendorCD = '0000000000000'
		and mio.StoreCD = '0000'
		and (tim.MainVendorCD <> '' and tim.MainVendorCD is not null)


			insert into M_ItemOrderPrice
		select  mainvendor,store, MakerItem, ChangeDate, Max(Rate), Max(OrderPriceWithoutTax), max( a),max(b),max(iopt),max(idt),max(uopt),max(udt) from
		(
		select  distinct
		   '0000000000000' as mainvendor
			 ...
			 ,@Date as udt
		from #tempItem tp
			inner join F_ITEM(getdate()) ms on tp.ITemCD = ms.ITemCD and tp.ChangeDate = ms.ChangeDate and ms.DeleteFlg = 0 
			where  (tp.MainVendorCD <> '' and tp.MainVendorCD is not null) and tp.MakerItem not in ( select mio.MakerItem from 
		M_ItemOrderPrice mio
		--inner join #tempItem tim on mio.MakerItem = tim.MakerItem and mio.ChangeDate = tim.ChangeDate
		where mio.VendorCD = '0000000000000'
		and 
		mio.StoreCD = '0000' )
	--and tp.MainVendorCD is not null and tp.MainVendorCD <> ''
	) ito
		where changedate is not null	group by MainVendor , Store , MakerItem, changedate order by MainVendor, MakerItem ,changedate
	--	insert into M_ItemOrderPrice
	--	select  distinct
	--	   '0000000000000'
	--		 ,'0000'
	...
	--		 ,@Date
	--	from #tempItem tp
	--		inner join F_SKU(getdate()) ms on tp.ITemCD = ms.ITemCD and tp.ChangeDate = ms.ChangeDate and ms.DeleteFlg = 0 
	--		where tp.MakerItem not in ( select mio.MakerItem from 
	--	M_ItemOrderPrice mio
	--	inner join #tempItem tim on mio.MakerItem = tim.MakerItem and mio.ChangeDate = tim.ChangeDate
	--	where mio.VendorCD = '0000000000000'
	--	and mio.StoreCD = '0000' and (tim.MainVendorCD <> '' and tim.MainVendorCD is not null))
	--and tp.MainVendorCD is not null and tp.MainVendorCD <> ''
