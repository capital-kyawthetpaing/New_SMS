...
BEGIN
	...
	[SKUCD] [varchar] (30) collate Japanese_CI_AS NOT NULL,
	...
	[WarehousingDate] ASC
		...
		SELECT
			...
			NULL,
			CASE 
				WHEN LastMonthQuantity IS NULL THEN 0
				ELSE LastMonthQuantity 
		...
		SELECT
			...
			--DW.WarehousingKBN AS [WarehousingKBN], 
			...
			CASE
				...
				WHEN DW.WarehousingKBN = 13 OR DW.WarehousingKBN = 14 THEN '移動元倉庫：' + DW.FromSoukoCD + ' ' + MS.SoukoName
				WHEN DW.WarehousingKBN = 11 OR DW.WarehousingKBN = 12 THEN '移動先倉庫：' + DW.ToSoukoCD + ' ' + MS.SoukoName
			...
			Case 
				WHEN MWK.CalculationKBN = 2 THEN Quantity
		...
		left outer join M_Vendor mv ON mv.VendorCD = DW.VendorCD
		left outer join M_Customer MC ON MC.CustomerCD = DW.CustomerCD
		left outer join M_Souko MS ON MS.SoukoCD = DW.FromSoukoCD AND MS.SoukoCD = DW.ToSoukoCD 
		...
		SELECT
			...
			NULL,
			NULL,
			ThisMonthQuantity AS ThisMonthQuantity-------------------------------在庫数
		FROM D_MonthlyStock
