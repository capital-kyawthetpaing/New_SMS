...
BEGIN
    ...
    SELECT DH.PurchaseNO
          ...
          + (CASE ISNULL(DO.AliasKBN,0) WHEN 2 THEN ' å‰íÜ' ELSE '' END) AS VendorName 
          ,DH.CalledVendorCD
          ,(SELECT top 1 A.VendorName
          FROM M_Vendor A 
          ...
          ,DA.VendorDeliveryNo
          ,ISNULL(DO.OrderPerson,'') + (CASE ISNULL(DO.AliasKBN,0) WHEN 1 THEN ' ól' ELSE '' END) AS OrderPerson
          ...
          ,(SELECT 'áÑ' + A.TelephoneNO + ' FAX' + A.FaxNO FROM M_Control AS A WHERE A.[MainKey] = 1) AS TEL
