...
insert into #tempItem
	select * FROM OPENXML (@DocHandle, '/NewDataSet/test',2)
			...
			Update 
			msk set
			msk.YearTerm = fsk.LastYearTerm
			,msk.Season =fsk.LastSeason
			,msk.CatalogNO = fsk.LastCatalogNO
			,msk.CatalogPage =fsk.LastCatalogPage
			,msk.CatalogText = fsk.LastCatalogText
			,msk.InstructionsNO = fsk.LastInstructionsNO
			,msk.InstructionsDate =fsk.LastInstructionsDate
			,msk.UpdateDateTime = @Date
			,msk.UpdateOperator =@Opt
			
			from M_SKUInfo msk
			inner join F_SKU(getdate()) fsk on msk.AdminNO = fsk.AdminNO
			inner join #tempItem tim on tim.ITemCD = fsk.ITemCD and tim.ChangeDate = fsk.ChangeDate and fsk.DeleteFlg =0
