...
BEGIN
          ...
          ,(SELECT CONVERT(varchar, MAX(DP.CollectClearDate), 111) 
           ...
           ,(CASE DH.JuchuuKBN WHEN 2 THEN (SELECT M.DenominationName FROM M_DenominationKBN AS M
                ...
                ELSE (SELECT M.DenominationName FROM M_DenominationKBN AS M
                       WHERE M.DenominationCD = DH.PaymentMethodCD) END) AS DenominationName    --入金方法
            
            ,(CASE WHEN ISNULL(DCB.CollectAmount,0)=0 THEN '未入金' 
                WHEN ISNULL(DCB.CollectAmount,0) <> DH.JuchuuGaku THEN '一部入金'
                WHEN ISNULL(DCB.CollectAmount,0) >= DH.JuchuuGaku THEN '入金完了'
    ...
    LEFT OUTER JOIN (SELECT D.JuchuuNO, SUM(DCB.CollectAmount) AS CollectAmount
            ...
            WHERE D.DeleteDateTime IS NULL
            GROUP BY D.JuchuuNO
            ) AS DCB ON DH.JuchuuNO = DCB.JuchuuNO   --入金状態
    ...
    IF @SalesDateFrom <> '' OR @SalesDateTo <> ''
    BEGIN
        DELETE FROM #TableForJuchuuShoukai
        WHERE SalesDate IS NULL;
    END

    IF @BillingCloseDateFrom <> '' OR @BillingCloseDateTo <> ''
    BEGIN
        DELETE FROM #TableForJuchuuShoukai
        WHERE BillingCloseDate IS NULL;
    END

    IF @CollectClearDateFrom <> '' OR @CollectClearDateTo <> ''
    BEGIN
        DELETE FROM #TableForJuchuuShoukai
        WHERE CollectClearDate IS NULL;
    END
