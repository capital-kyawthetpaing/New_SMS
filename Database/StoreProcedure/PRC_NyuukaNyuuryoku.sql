...
BEGIN
    ...
    BEGIN
        ...
        --【D_ArrivalPlan】    分割分削除（Delete）Table転送仕様Ｃ②
    ...
    BEGIN
        ...
        DECLARE CUR_TABLE CURSOR FOR
            SELECT tbl.ArrivalPlanNO, tbl.StockNO, tbl.ReserveNO, tbl.ArrivalSu
                  ...
                  ,(SELECT DR.ReserveSu - tbl.ArrivalSu    --元のレコードのReserveSu - 明細入荷数(区分が引当の画面に表示される引当数と入荷数)  
        ...
        -- ========= ループ内の実際の処理 ここから===
            ...
            BEGIN
                ...
                UPDATE [D_Reserve] SET
                       [ReserveSu]            = [ShippingPossibleSU] + @tblArrivalSu   --未確認
        ...
        --【D_ArrivalPlan】Update   Table転送仕様Ｃ,Ｃ①
        UPDATE [D_ArrivalPlan] SET
