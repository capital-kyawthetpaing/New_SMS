 BEGIN TRY 
 Drop Procedure dbo.[Amazon_API_Insert_NextToken]
END try
...
-- =============================================
CREATE PROCEDURE [dbo].[Amazon_API_Insert_NextToken]
...
BEGIN
    ...
    -- Insert statements for procedure here
			...
			declare	@Date as DateTime= getdate() 
	             ...
	             set @val=1;
	             End
				 declare @maxSEQDetails as int =(select Max(ISNULL(D_APIDetail.SEQ,0)) from D_APIDetail where InportSEQ = (select max(InportSEQ) from D_APIDetail) );
				 --declare @MaxSEQLIST as int = (select Max(ISNULL(D_AmazonList.InportSEQRows,0)) from D_AmazonList );
				 --declare @maxSEQJuchuu as int =  (select Max(ISNULL(D_AmazonJuchuu.InportSEQRows,0)) from D_AmazonJuchuu);
				 ---Author - PTK
				 ---Effective tables   
...
--2  		
				 ...
				 Update #temp set SEQ = (seq+@maxSEQDetails);
...
--5	 
			...
			--	declare @xmldetail as xml =N''
				 ...
				 exec sp_xml_removedocument @DocHandledetails;
				  ...
				  update #tmp set inportSeq =@val ,  InportSEQRows = (InportSEQRows + @maxSEQDetails)
