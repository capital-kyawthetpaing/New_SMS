...
insert into #tempItem
	select * FROM OPENXML (@DocHandle, '/NewDataSet/test',2)
			...
			exec sp_xml_removedocument @DocHandle;
			 ...
			 Update mjo
			 set ChangeDate =  tim.ChangeDate
			 ,SKUCD = ms.SKUCD
			 ,Rate = tim.Rate
			 ,PriceWithoutTax = tim.OrderPriceWithoutTax
			 from 
			M_JanOrderPrice  mjo 
			inner join F_SKU(getdate()) ms on  mjo.AdminNO = ms.AdminNO AND ms.DeleteFlg =0
			inner join #tempItem tim on mjo.ChangeDate = tim.ChangeDate AND  ms.ITemCD = tim.ITemCD 
			where  
			mjo.VendorCD = '0000000000000'
		   ...
		   '0000000000000'
			...
			,tim.ChangeDate
			 ...
			 ,@Date
			from #tempItem tim 
			inner join M_SKU ms on tim.ITemCD = ms.ITemCD and tim.ChangeDate = ms.ChangeDate and ms.DeleteFlg = 0
			 where tim.ITemCD not in (select ms.ITemCD  from 
			M_JanOrderPrice  mjo 
			inner join F_SKU(getdate()) ms on  mjo.AdminNO = ms.AdminNO AND ms.DeleteFlg =0
			inner join #tempItem tim on mjo.ChangeDate = tim.ChangeDate AND  ms.ITemCD = tim.ITemCD 
			where  
