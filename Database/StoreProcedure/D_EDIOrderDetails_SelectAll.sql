...
BEGIN
    ...
    SELECT DE.EDIOrderNO
          ...
          ,(SELECT top 1 M.VendorName FROM M_Vendor AS M 
            WHERE M.VendorCD = D.VendorCD
            AND M.ChangeDate <= CONVERT(date,D.ImportDateTime)
            AND M.DeleteFlg = 0 
            ORDER BY M.ChangeDate desc) AS VendorName
          ,CONVERT(varchar,D.ImportDateTime,111)+' '+SUBSTRING(CONVERT(varchar,D.ImportDateTime,108),1,5) AS ImportDateTime
          ,1 AS DelFlg
          
    INTO #TableForEDIKaitouNouki
    FROM D_EDIDetail AS DD
    INNER JOIN D_EDIOrderDetails AS DE
    ON DE.EDIOrderNO = DD.EDIOrderNO
    AND DE.EDIOrderRows = DD.EDIOrderRows
