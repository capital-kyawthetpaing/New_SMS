...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
							...
							Begin
							Delete  from M_SKUTag where AdminNO in (select AdminNO  from M_SKU where ITemCD  in (select  ITemCD  from #tempItem ))

								 --update #tempItem set TagName01=2 where ITemCD = 'ac102-5p'
							...
							End
								 ...
								 --insert into M_SKU
								--select * from #tempItem  where ITemCD = 'ac102-5p'
																										
								--	Delete  from M_SKU where AdminNO =1
								--select *  from M_SKUTag
				--SELECT COUNT(*) 
	...
	--		drop table CountSetting								
			...
			--select *  from M_SKU where AdminNO = '569'
END
